...
""".trimIndent()
    ...
    fun `locked api chapter maps to non-readable chapter data`() {
        ...
        assertEquals("Locked reader URL must stay non-readable", LOCKED_READER_URL, chapter.readerUrl)
    ...
    fun `locked api chapter materializes locked SChapter with lock-marked name`() {
        ...
        assertEquals("Locked SChapter URL must stay non-readable", LOCKED_READER_URL, chapter!!.url)
    ...
    @Test
    fun `received ordering of locked and unlocked records is preserved`() {
        val records = listOf(
        ...
        val chapters = records.mapNotNull { it.toSChapter(slug, baseUrl) }

        assertEquals("All three received records must map into the list", 3, chapters.size)
        assertEquals("Order 18 must stay first", "🔒 Capítulo 18", chapters[0].name)
        assertEquals("Locked 17 must stay between 18 and 16", "🔒 Capítulo 17", chapters[1].name)
        assertEquals("Order 16 must stay last", "Capítulo 16", chapters[2].name)
        assertTrue("Unlocked reader URLs must be readable", chapters[2].url.startsWith("/leer/"))
    }
