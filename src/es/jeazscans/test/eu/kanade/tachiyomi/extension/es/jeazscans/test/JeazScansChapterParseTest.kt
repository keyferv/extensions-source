...
import eu.kanade.tachiyomi.extension.es.jeazscans.LOCKED_READER_URL
...
""".trimIndent()
    ...
    fun `locked api chapter maps to non-readable chapter data`() {
        ...
        assertTrue("Locked reader URL must stay non-readable", chapter.readerUrl.startsWith("$LOCKED_READER_URL/"))
    ...
    @Test
    fun `locked api chapter without parseable number is kept in the list`() {
        val lockedNoNumber = ChaptersApiChapter(
            id = 22811,
            number = null,
            title = "Capítulo 17",
            publishedAt = "Hoy",
            isLocked = true,
            price = JsonPrimitive(120),
            paymentUntil = "2026-08-06 03:28:42",
        )

        val chapter = lockedNoNumber.toChapterData(slug, baseUrl)

        assertNotNull("Locked record without a number must not be dropped", chapter)
        assertEquals("Chapter number must be derived from the title", 17.0f, chapter!!.chapterNumber, 0.001f)
        assertTrue("Locked reader URL must stay non-readable", chapter.readerUrl.startsWith("$LOCKED_READER_URL/"))
        assertEquals("🔒 Capítulo 17", chapter.displayName())
    }

    @Test
    fun `locked api chapter without number or numeric title falls back to id`() {
        val lockedSpecial = ChaptersApiChapter(
            id = 22811,
        ...
        val chapter = lockedSpecial.toChapterData(slug, baseUrl)

        assertNotNull("Malformed number must not drop a locked record", chapter)
        assertEquals("Chapter number must fall back to the id", 22811.0f, chapter!!.chapterNumber, 0.001f)
        assertEquals("🔒 Edición especial", chapter.displayName())
    }

    @Test
    fun `all locked records and unlocked records remain in order`() {
        val records = listOf(
            ChaptersApiChapter(id = 22813, number = "18.00", title = "Capítulo 18", isLocked = true, price = JsonPrimitive(120), paymentUntil = "2026-08-06 03:28:42"),
            ChaptersApiChapter(id = 22811, number = null, title = "Capítulo 17", isLocked = true, price = JsonPrimitive(120), paymentUntil = "2026-08-06 03:28:42"),
            ChaptersApiChapter(id = 22809, number = "16.00", title = "Capítulo 16", isLocked = false),
        )

        val chapters = records.mapNotNull { it.toChapterData(slug, baseUrl) }

        assertEquals("All received locked and unlocked records must remain", 3, chapters.size)
        assertEquals("Order 18 must stay first", "🔒 Capítulo 18", chapters[0].displayName())
        assertEquals("Locked 17 must stay between 18 and 16", "🔒 Capítulo 17", chapters[1].displayName())
        assertEquals("Order 16 must stay last", "Capítulo 16", chapters[2].displayName())
        assertTrue("Unlocked reader URLs must be readable", chapters[2].readerUrl.startsWith("/leer/"))
    }

    @Test
    fun `locked records receive unique non-readable urls`() {
        val records = listOf(
            ChaptersApiChapter(id = 26956, number = "26", title = "Capítulo 26", isLocked = true),
            ChaptersApiChapter(id = 24186, number = "25", title = "Capítulo 25", isLocked = true),
        )

        val chapters = records.mapNotNull { it.toChapterData(slug, baseUrl) }

        assertEquals(2, chapters.size)
        assertTrue(chapters.all { it.readerUrl.startsWith("$LOCKED_READER_URL/") })
        assertTrue("Locked chapter URLs must be unique", chapters[0].readerUrl != chapters[1].readerUrl)
    }

    // ── Locked chapter display name ────────────────────────────────────────────

    @Test
    fun `locked display name shows only lock symbol and title`() {
        val data = ChapterData(
            ...
            isLocked = true,
            priceCoins = 120,
            paymentUntilEpoch = System.currentTimeMillis() + 11 * 3600_000L + 24 * 60_000L,
        )

        assertEquals("🔒 Capítulo 15", data.displayName())
    }

    @Test
    fun `locked display name omits price and countdown regardless of deadline`() {
        val elapsed = ChapterData(
            readerUrl = LOCKED_READER_URL,
            chapterNumber = 15.0f,
            name = "Capítulo 15",
            dateUpload = 0L,
            isLocked = true,
            priceCoins = 120,
            paymentUntilEpoch = System.currentTimeMillis() - 60_000L,
        )

        val name = elapsed.displayName()

        assertTrue("Price must not leak into the display name", !name.contains("monedas"))
        assertTrue("Countdown must not leak into the display name", !name.contains("Gratis"))
    }
