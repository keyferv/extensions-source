...
/**
 * Solves Cloudflare Turnstile / managed challenge by loading the target URL in a
 ...
 *
 * On success the `cf_clearance` cookie is set in the shared [CookieManager],
 * which is picked up by the OkHttp cookie jar on subsequent requests.
 *
 * Adapted from YuriGarden's CloudflareResolver.
 */
object CloudflareResolver {
    ...
    fun resolve(loadUrl: String, userAgent: String? = null): Boolean {
        val cookieManager = CookieManager.getInstance()
        if (hasClearance(cookieManager, loadUrl)) return true
        ...
        handler.post {
            ...
            poll = Runnable {
                ...
                if (hasClearance(cookieManager, loadUrl)) {
                    latch.countDown()
                } else {
        ...
        return hasClearance(cookieManager, loadUrl)
    }
